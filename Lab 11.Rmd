...
- How can we improve on this? 
  ...
  using expert knowledge and research design
...
Ommit variable bias
expression of difference in means
Selection bias term = impact x inbalance
...
so we run the restricted regression
...
\begin{align}
\hat{bias}=
\underbrace{\frac{cov(E[D|X],E[Z|X])}{Var(E[D|X])}}_{\hat{\delta}} \underbrace{\frac{cov(E[Y|X,D],E[Z|X,D])}{Var(E[Z|X,D])}}_{\hat{\gamma}} \nonumber 
\end{align}
...
r squared in the shared variance in y that can be explained by z|DX
partial $R^2_{Y\sim Z|D,X} = 1 - \frac{Var(E[Y|D,X])}{Var(Y)} = cor(E[Y|D,X],Y)^2$

bias term is the square room of = 
...
sd(t hat res) is observable and can be found from regression
\begin{align}
...
first way: Imagine wealth explained all variance in attitudes, how much of the covariate would wealth have to explain for our bias term to be large.
...
. . .
  ...
  share of d thats explained by Z must be the same as the share of y thats explained by d
  
  - How sensitive are our results to including an extreme confounder s.t.,  $R^2_{Y\sim Z|D,X} = 1$
  

second : if the r squared y dx and the r squared z|xd how strong would the association have to be in order to reduce the estimated ATE
...
# a confounder that explains 13% of treatment and outcome in order to drive estimated effect to zero
...
```
benchmarking at female
k:d how much greater is the confounder associated than female
q is the r squared that drives our results to zero
alpha is our CI
