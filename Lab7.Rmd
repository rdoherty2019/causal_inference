...
# Covariate Balance
```{r 5-lalonde-balance}
#Matching improves balance

#trt indicator
trt <- lalonde$treat==1

#Specify data set, minus first column
# 2 means by column
#tapply do this funtion
...
#store the indenfied observation
#subclas identify whihc pair you are in
exact.data <- match.data(em.match) #N=25 observations

head(exact.data[order(exact.data$subclass),c(1,8:12)])
#cant exact match on continous covairates,
```
...
exact.data$id <- paste(exact.data$subclass,
                       exact.data$treat)
#new column of sub groups, each with a unique id, 02 (contorl subgroup 2)
#sampling a row from each subgroup
#lapply reutrns a list, unlist, returns a numeric object
rand.units <- unlist(
...
#difference in outcomes in each subclass
...
```{r 5-exact-formula2}
#ATE
#for each subclass, the difference in means, 1978
...
#procude estimates that are opposites
coefs
```

# Regression Model 
What seems problematic with that approach?
We have no double robust
. . .

```{r 5-exact-est}
#conditioning on covairates, exact data weights
#weight control units see below
...
# Nearest Neighbor
```{r 5-matchit}
#matchit to specify model
#discard if we can't find a mathc, here we discard the control
...
```{r 5-cem-start}
#cem package
#dropping the outcome re78
#but will match on every other covariate
cem.match <- cem(treatment="treat",data=lalonde,
                 drop="re78")
cem.match #395 strata

#weights are defaulted to w
...
# Estimates
```{r 5-tweak-results}
#tweaking has a large effect on point estimate
coefs
coefs2
```
...
#take the columns
#for each treated unit give me the mahal distance, and take the smallest control
...
#applying the minimum for each column
#apply mean
...
#weights
  #treated are 1
  #contro are 1-treat/1- inverse prosenity score
...
match.data <- subset(lalonde,treat==1)
#take the difference and then find the minimum ctronl units
