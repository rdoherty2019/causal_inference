...
#assumptions you can easily falsify,
- Full rank regression matrix (variation in $X$)
  if voilated drop one of the covariates
- Zero conditional mean of the errors ($\mathbb{E}[\epsilon_i | X] = 0$)
  violation of ommited variable basic
. . .
#Variance
...
```{r fig.show='hide'}
#points for every value of x give me a value for fit 1
p; points(	X,predict(fit1), type="l", col="red", lwd=2)
#semi colin is new line
#text opbject at 5,13
text(	5,13, expression(X[1]*beta[1]), col="red")
round(mean(residuals(fit1)*X),5) # Orthogonality
#discover values of coefficents such that the residuals are ortho to X
p <- recordPlot()
```
...
#get rsiduals around the example 4.05
resid.example <- mean(residuals(fit1)[X>=4&X<=4.1])
#the y at that point is going to be the sum of the precited values + the errors
...
```{r}
#recovering linearity we need to add X^2
#always an assumptions unless we have information such as expert in area
#only time we can build in linearity is in discrete X's
fit2 <- lm(Y~X+I(X^2))
X.ord <- X[order(X)]
...
#first 500 are 1 next 500 are 0
X <- c(rep(1,N/2), rep(0,N/2)); P <- .2 + .3*X
#each units probability of being 1 is a function of x
...
print(v20); print(v21) #variances are the same
```
...
## Go se video of the Mondey 3/30 Lecture for this proof
...
- Using Bayes rule $Pr[D_i = 1|X_i = x] = \frac{Pr[X_i = x|D_i=1] Pr[X_i=x]}{\sum_x Pr[X_i = x|D_i=1] Pr[X_i=x]}$
- Which means that the ATT is a propensity-score weighted function of the CATEs: $ATT = \frac{\sum_x \tau_x  Pr[X_i = x|D_i=1] Pr[X_i=x]}{\sum_x Pr[X_i = x|D_i=1] Pr[X_i=x]}$
...
- constant treatment effects $\tau_x = \tau$ for all X or
  - You can pull out the TAU of the summation
- unconditional independence 
  - D_i is orthoganal to X you can pull them out and the probability of being Xs cancel out
...
- $Pr[D_i = 1|X_i = x](1-\Pr[D_i = 1|X_i = x])$ is maximized when $Pr[D_i = 1|X_i = x]=1/2$
  - When there are equal number of treatments and control in the covirates these value will be upweights where unbalanced covairates will be down weighted
- Regression weights produce an effective sample different from the observed sample
  - affects the weighting for the observations that end up in your sample



# Effective Samples
- Let's check the properties of your effective sample in regression.
- The key result is a function of these weights: $\hat{\rho}_{reg}\,{\buildrel p \over \to}\,\frac{E[w_i \rho_i]}{E[w_i]}$
