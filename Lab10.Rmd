...
yi and our treatment variable
Second stage: $Y_i = \alpha_0 + \alpha_1 D_i + \epsilon_i$ 

relationship between instrument and treatment indicator
First stage: $D_i = \beta_0 + \beta_1 Z_i + \mu_i$

- Exogenous instrument 
Potential outcomes for d1 and d0 are independent of Z
...
- Wald estimator 
  ...
  - Placebo regressions for exclusion restriction
    Assumption like conditional independence
  
- Preliminaries on 2SLS estimator
  - More in class
  - Heteregeneous treatment effeccts
  - Two papers
...
The ITT is helpful but not the instrument we are interested
...
&= \frac{Cov(Y,Z)}{Cov(D,Z)} =\frac{\sum_{i=1}^N (z_i - \bar{z}) (y_i - \bar{y})}{\sum_{i=1}^N (z_i - \bar{z}) (x_i - \bar{x})}  \nonumber 
\end{align}
...
\begin{align}
\rho&=\frac{\sum_{z_i=1} (z_i y_i - z_i \bar{y})}{n_1} / \frac{\sum_{z_i=1} (z_i x_i - z_i \bar{x})}{n_1}  \nonumber \\
&=\frac{\bar{y_1} -\bar{y}}{\bar{x_1} -\bar{x}}  \nonumber
\end{align}

Using the fact that $\bar{y} = \frac{n_1 \bar{y_1}+n_0 \bar{y_0}}{n}$
\begin{align}
\rho=\frac{\bar{y_1} -\bar{y_0}}{\bar{x_1} -\bar{x_0}}  \nonumber
\end{align}

Converges in probability to...
\begin{align}
=\frac{E[Y_i|Z_i=1]}{E[D_i|Z_i=1]} - \frac{E[Y_i|Z_i=0]}{E[D_i|Z_i=0]} \nonumber
\end{align}
...
```
Increase in earnings attributial to only assignment treatment
Local average treatment effect

# Variance

- The asymptotic standard error of the Wald estimates is derived from the limiting distribution of $\sqrt{n} (\bar{y_1} - \bar{y_0})(\bar{x_1}-\bar{x_0})$. 
- The numerator has a nondegenerate limiting distribution, while $(\bar{x_1}-\bar{x_0})$ converges to a constant. 
Will increase the more numbers you put there

- The standard error is therefore equal to $1/(\bar{x_1}-\bar{x_0})$ times the standard error of the numerator 
The compliance rate
...
```
Hypothesis test
Is there a difference in these two models
...
SE that is extremely off if you do it one by one
...
summary(first)
```
 PLaces with higher average protection from exportation have  lower settler morality                              
# Examine Output

```{r 6-show-2sls}
summary(iv2sls)
```
1% difference in GDP once you remove bias in the first stage                                
...
- Treatment: Segregation (level of dismilarity)
  - What percentage of blacks (or nonblacks) would have to move to another census tract in order for the proportion black in equal tract to be constant
      amount of people you would have to move to balance the proportion of black members of census tract
...
```
OLS place with a higher level of segregation have a higher level of income inequality
IV relationships
