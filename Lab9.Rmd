...
#unit specfic dummy and a time specific dummy
...
- Visualizing parallel trends (Visualize identification assumption)
...
```
had medicaid expansion had nine fewer deaths per 100,000, SE is large, so no causal effect

# Difference-in-means estimates by year #this is a cross sectional difference
```{r paper1-fd}
#store esimttes in list
dim_estimates <- c()
se_estimates <- c()
...
library(ggplot2)
#ggplot by year
#each value of the year us going to range from the min to the max
...
```
Differences are subject to change over time, obviously it is compounded
...
```
increase in death rate after 2001 when compared in 2001
...
```
compared to control states treated states have 8 less deaths but that is not statisitcall signifcant
Cluster level increases standard error
always remember to clster on unit level

# Testing Parallel Trends Setup
interact the treatment indicator with each year
```{r paper1-pll_test}
#generate placebo treatments
#making a treatment variable
#assign makes a treatment vairbale in the global enviroment
...
#storing the placebo DiD estimates in list
...
dat2 <- data.frame(year=c(1999:2001,2003:2016),
                  ...
                  se_estimates = se_estimates)
```
if parrallel trends hold we should see no change in parrellel trends before treatment
...
results <- data.frame(
 ...
 #since whites oare ommitted it is the coefficent
results$m1_est[1] = 
...
#just the variance, if you just looked at coefficents
results$m1_ses[1]<- sqrt(v_model['stricty', 'stricty'])
#the sqrt fo the vairance of (X) + varaince of (y) + 2* variance of (X,y)
...
results
```
This is what we would expect, when we drop virgina we see that voter rstriction laws do not increase voter turn out
Be careful of standard error, we have cluster varianced
...
for (j in 1:sims){
  ...
  star <- sample(gnames, size = length(gnames),
                 replace = TRUE)
  dat.star <- dat[unlist(lookup[star]),] #new data #data needs to be unlisted because star will return a list
